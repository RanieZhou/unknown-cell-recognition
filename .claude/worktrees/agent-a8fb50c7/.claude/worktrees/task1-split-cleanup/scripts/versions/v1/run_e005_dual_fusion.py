...
# Fusion params
LAMBDA = 0.2
TAU_G = 0.90  # 90th percentile threshold for GRN
...
def load_scores():
    log("Loading scores...")
    expr_df = pd.read_csv(os.path.join(OUTPUT_DIR, "E005_expression_scores.csv"))
...
def normalize_scores(df):
    ...
    # ECDF normalize expr_fused
    val_expr = df.loc[val_mask, "expr_fused"].values
    df["expr_pct"] = ecdf_normalize(df["expr_fused"].values, val_expr)

    # ECDF normalize grn_distance_score
    val_grn = df.loc[val_mask, "grn_distance_score"].values
...
def compute_dual_fusion(df):
    log("Computing dual fusion score...")

    # τg is the TAU_G percentile of grn_pct on val_known
    val_mask = df["E005_split"] == "val_known"
    tau_g_value = np.percentile(df.loc[val_mask, "grn_pct"].values, TAU_G * 100)
    log(f"  τg (GRN {TAU_G*100:.0f}th pct on val_known): {tau_g_value:.4f}")

    # dual_fused = expr_pct + λ * max(0, grn_pct - τg)
    grn_bonus = np.maximum(0, df["grn_pct"].values - tau_g_value)
    df["dual_fused"] = df["expr_pct"].values + LAMBDA * grn_bonus

    log(f"  dual_fused range: [{df['dual_fused'].min():.4f}, {df['dual_fused'].max():.4f}]")

    return df, tau_g_value
...
def generate_summary(metrics, thresholds, df):
    ...
    summary_lines.extend([
        ...
        f"- λ = {LAMBDA}",
        f"- τg = {TAU_G}",
        f"- Expression weights: entropy={0.5}, distance={0.5}",
    ])
