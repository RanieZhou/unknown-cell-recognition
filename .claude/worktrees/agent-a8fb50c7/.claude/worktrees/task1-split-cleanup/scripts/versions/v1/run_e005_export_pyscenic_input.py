...
SCORES_PATH = os.path.join(OUTPUT_DIR, "E005_expression_scores.csv")
...
def main():
    ...
    # 5. Filter genes: keep genes expressed in at least 1% of cells
    min_cells = int(0.01 * expr_matrix.shape[0])
    gene_mask = (expr_matrix > 0).sum(axis=0) >= min_cells
    expr_filtered = expr_matrix[:, gene_mask]
    genes_filtered = gene_names[gene_mask]
    log(f"  After gene filter (>={min_cells} cells): {expr_filtered.shape[1]} genes (from {len(gene_names)})")
    ...
    log("=" * 60)
    log("Step 2 DONE. Next: run_e005_pyscenic_cli.ps1")
    log("=" * 60)
